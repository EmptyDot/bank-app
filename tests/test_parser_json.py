...
class TestCustomerParserJson:
    def test_load_customers(self):
        customers = CustomerParserJson().load_customers("tests/data/test_saved_customers_load.json")
        assert customers == get_customer_list()

    def test_load_customers_wrong_file(self):
        assert CustomerParserJson().load_customers("tests/data/does_not_exist.json") is None
    ...
    def test_create_customer(self):
        ...
        bob.accounts = [Account(acc1_num, acc1_balance), Account(acc2_num, acc2_balance)]
    ...
    def test_save_customers(self):
        ...
        assert CustomerParserJson().save_customers(customers, "tests/data/test_saved_customers_save.json")

    def test_save_customers_no_customers(self):
        assert CustomerParserJson().save_customers([], "tests/data/test_saved_customers_save.json") is False
